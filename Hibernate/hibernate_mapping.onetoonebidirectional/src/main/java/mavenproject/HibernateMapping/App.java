...
/**
 * Main1 class for Hibernate mapping operations.
 */
public class App {
	public static void main(String[] args) {
		...
		Query q1 = s.createQuery("from Studnet123 ORDER BY sname");
		...
		Query countQuery = s.createQuery("select count(*) from Studnet123");
		Long count = (Long) countQuery.uniqueResult();
		System.out.println("Total number of students: " + count);

		// Query to group students by city and count them
		String hql = "SELECT scity, count(*) FROM Studnet123 GROUP BY scity";
		...
		// Name Query
		Query query1 = s.getNamedQuery("findStudnetId");
		query1.setInteger("Id1", 2);
		List<Student123> list = query1.list();
		for (Student123 bk : list) {
			System.out.println(bk);
		}
